...
def render(scene, im_arr, fov, pixel_size):
    ...
    for i in range(width // pixel_size):
        col = i * pixel_size
        current_angle = utils.lerp(col / width, fov_rads / 2, -fov_rads / 2)
        d = raycast(scene.player, current_angle, scene.level.level_map)
        # avoid fish eye effect by using perspective projection
        persp_d = math.cos(current_angle) * d if current_angle != 0 else d
        # color for the wall
        ...
        wall_height_pixels = min(
            int(round((WALL_HEIGHT_MTS / persp_d) * height)), height
        )
        y_start = (height - wall_height_pixels) // 2
        rgb_color = utils.float2uint8(color)
        im_arr[y_start:height-y_start, col:col+pixel_size] = rgb_color
